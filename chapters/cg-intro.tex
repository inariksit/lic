...
it is not a meaningful question to define the expressivity of ``CG'', but rather CG-1 \cite{karlsson1995constraint}, CG-2 \cite{tapanainen}, VISLCG, VISL CG-3, or any other less mainstream implementation.

But can we emulate some kind of behaviour that would put it into the Chomsky hierarchy? 

\cite{tapanainen99phd} defines finite-state constraint languages as having `` contextual tests that are expressed in terms of regular languages''.
