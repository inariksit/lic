...
But our task now is to {\em generate} the original sentence. 
Here comes the final trick: we will apply the rules to something we call {\em symbolic sentence}. 
...
The width of the last rule is three: one condition to the left of the target, one to the right, so we create a symbolic sentence of three words.
...
This was a rather simple example of a conflict. Furthermore, the order was not really crucial. We will show another example, where we illustrate why is it important to create new variables, and how conditions can affect the state of the variables, but in a more limited way.
