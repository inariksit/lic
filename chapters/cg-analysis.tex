...
This can be seen as a generalisation of the concept \emph{bleeding order} in generative phonology \todo{cite Kiparsky?}; however, we include also rule-internal conflicts.
...
In the following, we list improvements 
The following is a list of improvements to the current features; we 
...
Below, we envision some properties that 

; however, we would be interested in getting feedback from actual grammarians and adding features based on what is needed.
...
\paragraph{Suggesting alternative orders} \todo{I might just be hallucinating and this is totally not feasible with SAT. But I got the idea, so documenting it here in case.} 
