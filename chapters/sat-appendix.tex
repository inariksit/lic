...
\cientificaAdj \impl \neg{}\masAdj &  \cientificaAdj \impl \masAdv \wedge \neg{}\masAdj \\
\end{array}
\end{equation}
...
The operators can be applied to a single condition, such as \cgrule{IF (NOT 1 noun)}, as well as linked conditions, such as \cgrule{IF (-2 det LINK NOT *1 noun)}.
Inversion cannot be applied to a BARRIER condition. If one wants to express \cgrule{IF (*1 foo BARRIER $\neg$bar)}, `try to find a \emph{foo} until you see the first item that is not \emph{bar}' a set complement operator must be used: \cgrule{(*) - bar}.
...
For an unordered analogue, we can ask a more complex question: ``out of all the rules that target this cohort, which one is a best fit \emph{with other rules that will apply to this whole sentence}?''
...
\paragraph{Difference between ``condition didn't hold in the first place'' and ``two rules have the same condition and conflicting action''}


\begin{figure}
  \begin{itemize}
   \item[] \begin{verbatim}REMOVE adv IF (-1 n) ;
REMOVE adj IF (-1 n) ;\end{verbatim}
   \item[] \begin{verbatim}REMOVE adj IF (-1 n) ;
REMOVE adv IF (-1 n) ;\end{verbatim}
   \end{itemize}
