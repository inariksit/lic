...
\paragraph{From disambiguation to generation}
...
If all variables start off as true, then we have, in effect, reimplemented the logical encoding in \cite{lager_nivre01}. 
This encoding is not a SAT-problem: there is no search involved, it is simply a matter of manipulating Boolean expressions. 
...
But our task now is to {\em generate} the original sentence, that will pass through the rules. 
...
Let us look at the following rules. For simplicity, we assume that the full set of readings in the language is $\{ \t{det def, noun sg, noun pl, verb sg, verb pl} \}$.


\begin{verbatim}
REMOVE verb IF (-1 det) ;
REMOVE verb IF (-1 det) (1 noun) ;
\end{verbatim}
...
The width of the last rule is three: one condition to the left of the target, one to the right, so we create a symbolic sentence of three words. Below, we show the symbolic sentence, consisting of three symbolic words: {\em w1, w2} and {\em w3}.
...
symbolic words {\em w2} and {\em w3}; the first word {\em w1} is excluded, 
because it has no preceding word.
Below we see the newly created variables after the first rule application:
...
\begin{itemize}
\item target readings: at least one reading with the \emph{verb} tag in the target position {\em w2}
\item ambiguity: at least one reading without the \emph{verb} tag in the target position; if there are only \emph{verb} readings in the target, the rule would not fire.
...
We can see that some of these requirements can be fulfilled: there are models where the target readings are in place in {\em w2}. If the rule had been \t{REMOVE verb}, that is, unconditionally remove all verbs, then this requirement could not be fulfilled. But so far we have not run into the wall. 
The second requirement for ambiguity is no problem either: there have been no rules that target the other readings of {\em w2}, so we are free to choose anything.
As for conditions, the part about \t{(1 noun)} can be fulfilled: nothing prevents {\em w3} from being a noun. But the first condition, \t{(-1 det)}, cannot be true with the target being verb: we have already had a rule that has prohibited such combination. 
Thus, when we try to ask for a solution where all these requirements hold, we will get a conflict. This shows us that the rule \t{REMOVE verb IF (-1 det) (1 noun)} cannot ever apply, if the rule \t{REMOVE verb IF (-1 det)} has been already applied. 
...
\paragraph{Example: no conflict}
The previous was a simple example of a conflict: two rules target the same reading, and the conditions were chosen such that the first rule applies in more cases than the second.
...
To begin, let us only look at the first and the last rule. Like in the previous example, they target the same reading, but now the order is good: the first rule is the one with a narrower condition, which means that it is possible for a sequence to pass through it, and still have the verb reading in {\em w2} intact. %---it just needs to have something else in addition to the determiner in {\em w1}
The following is an example of such sequence---if we stopped after $r1$ and asked for a solution that triggers $r3$, we might get just that.
...
To recap, we passed through $r1$ with the case ``condition does not hold, target not removed''. Now, let us add $r2$: \t{SELECT det IF (1 verb)}. 
We must, in fact, select the determiner: 
the condition of $r2$ happens to be the target of $r3$, so it must hold. 
...
At time $t1$, the symbolic word {\em w1} had to be ambiguous---that is, one of the variables $w1_{\noun\sg}, w1_{\noun\pl}, w1_{\verb\sg}, w1_{\verb\pl}$ must be true. 
But at time $t2$, all of the variables $w1'_{\noun\sg}, w1'_{\noun\pl}, w1'_{\verb\sg}, w1'_{\verb\pl}$ must be false.
We have arrived to the situation where {\em w1} at time $t1$ does not trigger $r1$, 
and {\em w1} at time $t3$ triggers $r3$.
Since the clauses by $r1$, $r2$ and $r3$ operate on different variables, there is no conflict.
