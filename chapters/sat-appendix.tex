...
To ensure that each cohort contains at least one true variable, we add the clauses in \ref{eq:atleastOneTrue}. 
...
We begin by introducing the unordered scheme. For basic rules, this corresponds to the encoding in \cite{lager98}; each analysis is given a single variable, and the rule application is unordered. 
...
\cientificaAdj \Longrightarrow \neg{}\masAdj &  \cientificaAdj \Longrightarrow \masAdj \wedge \neg{}\masAdv \\
\end{array}
