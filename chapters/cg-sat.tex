...
have another potential noun in the sentence, but there is no other rule which targets 
its other readings. We can discard only the conflicting instances: we prevent 
\t{REMOVE n} from applying to $\casaN$, but it still may apply to other nouns.
...
In other words, the goodness of a solution is measured in how many of 
the helper variables are true.

What happens if a given $v$ is false? The SAT-solver has received all its clauses 
with the added $\neg{}v$. 
\todo{Does it make sense to translate this back into an implication and explain on that level?}
If we know that $\neg{}v \vee \neg\laDet \vee \neg\casaV$ 
and $\neg{}v$, we see immediately that the whole disjunction evaluates to true.
%can discard the rest of the clause---it is a disjunction, and if $\neg{}v$ is true, then the whole disjunction is true.
This means that if $v$ is false, then the SAT-solver can discard the rest of the clause; namely, the part that came from the rule application. %does not need to care about its respective clause.
Conversely, if $v$ is true, then its negation $\neg{}v$ is eliminated from the clause, 
and the SAT-solver will have to deal with the original $\neg\laDet \vee \neg\casaV$.
