...
In contrast, a standard, sequential CG would have just applied the first rule and stopped there. If, later on, the determiner is removed, there is no way to restore the verb. If we add one more rule, \t{REMOVE v IF (1 adj)}, then we will only get the model in column 1 of Figure~\ref{fig:modelsTwoRules}. Notice that none of the rules has targeted the word \la{}; still it has become unambiguous.
