...
\begin{table}
  \centering
  \begin{tabular}{ r | c c c | c c c }
 %      & \multicolumn{3}{c}{Full novel: 384,155 words} & \multicolumn{3}{c}{Longest sentence: x words} \\
...
\begin{table}
  \centering
  \begin{tabular}{ r | c c c | c c c }
       & \multicolumn{3}{c}{Longest sentence: 283 tokens} & \multicolumn{3}{c}{Longest sentence split at semicolons: x + 126 + y tokens} \\
     \textbf{\# rules} & \textbf{\satcgMax}  & \textbf{\satcgOrd} & \textbf{VISL CG-3} 
                                        & \textbf{\satcgMax}  & \textbf{\satcgOrd} & \textbf{VISL CG-3} \\ \hline
   ...
   \end{tabular}
  \caption{Experiments with sentence length: parsing the longest sentence in Don Quijote. On the left, the complete 283-token sentence; on the right, split in three parts.}
  \label{table:time}
\end{table}
...
Don Quijote contains, on average, longer sentences: \todo{make a plot}.

Table~\ref{table:time} shows execution times compared to VISL CG-3;

The left column contains the time for the full novel, and the right column for the longest sentence, of 126 tokens\footnote{y que él, ansimesmo, en los años de su mocedad, se había dado a
