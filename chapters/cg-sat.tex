...
So we do counting instead. When we build clauses, we associate to each of them a helper variable. Remember that it's all just disjunctions: $\neg\laDet \vee \neg\casaV$. 
Now, we add the variable $a$ to each clause, such that $[\neg{}a \vee \neg\laDet \vee \neg\casaV]$, and retain all the $a$s. 
Since the formula contains $\neg{}a$, if $a$ later on will be assigned true, 
the clause will be simplified to $[\neg\laDet \vee \neg\casaV]$. 
This means that the implication in it will be taken into account by the SAT solver.

Consider what happens if $a$ is not true: then $[\neg{}a \vee \neg\laDet \vee \neg\casaV]$ is known to be true because of $\neg{}a$, and the whole clause will be removed; SAT-solver can find a solution without ever satisfying the original $\neg\laDet \vee \neg\casaV$.
