...
The question ``what can CG express?'' depends completely on what is present in the initial input. 

Take the example of central embedding; a feature known to require a context-free grammar. If a given generative grammar would analyse correctly an arbitrary number of central embeddings (e.g. \emph{the dog \emph{the cat bit} died}), this would suggest that the grammar is (at least) context-free. In order to translate such requirement to CG, we have two questions to ask: 
\begin{inparaenum}
\item Can a CG grammar correctly disambiguate a sentence with arbitrary number of central embeddings?
...
Despite these difficulties, we argue that it is possible to emulate some kind of behaviour that would put CG into the Chomsky hierarchy.

%Assume we have a rule, which can disambiguate the word form \emph{who} as a relative pronoun instead of interrogative in the context ``N who V'', as a simple, local pattern matching task. 
%This rule will perform the correct disambiguation for the pronoun, even if the relative clause is centrally embedded; a feature known to not be regular. If all the other rules are successful, then the grammar has chosen a correct analysis for a sentence, which is not context-free.
