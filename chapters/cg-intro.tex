...
sleeps''. 
It contains three word forms, each followed by its \emph{readings}.
A reading contains one lemma, such as ``bear'', and a list of morphological tags, such as \emph{det def}.
...
Its task is to give correct analyses to the words given sentences,
...
The analysis is limited to moprhology and surface syntax; no hierarchical tree structure is provided.
CG rules usually operate on a low level of abstraction; instead of one rule to describe the possible structure of \emph{noun phrase},
CG would include many small rules each describing one possible kind of noun phrase.
Finally, the rules are independent
...
The analysis starts from a list of alternatives for each word, and proceeds by eliminating impossible or unlikely candidates.
...
Syntactic or dependency labels are functionally the same as morphological: \texttt{@Subj} is just one more tag in a reading.

It is possible to modify the morphological parser, or use a suitable post-processing, to add even more data. One could add semantic roles, or any other information that a lexicon may provide, such as animacy or frequency.
...
rule to remove a verb after a determiner and an adjective (\emph{the happy
bear sleeps}), and a third rule to remove verb after a possessive
pronoun (\emph{my bear sleeps}). A phenomenon that is expressed with one
