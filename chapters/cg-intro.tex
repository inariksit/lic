...
95-97 \% for syntax \cite{bick2000palavras,bick2003hybridCG_PSG,bick2006spanish}.
...
CG is one in the family of shallow and reductionist grammar formalisms. In this section, we briefly describe other formalisms of the same family, and provide a bit of historical context. In-depth details about CG in particular are presented in Section~\ref{properties}.
...
Karlsson \cite{karlsson1995constraint} provides a thorough overview on 
different approaches to the disambiguation problem, including \cite{hindle1989disamrules,herz1990localSyntacticConstraints}.
...
CG itself was introduced in 1990. Around the same time, two related formalisms were also proposed: Finite-State Intersection Grammar \cite{koskenniemi90}, also later called Parallel Constraint Grammar; like in (sequential) CG, a FSIG grammar contains of rules which remove impossible readings based on contextual tests.
But FSIG rules usually target syntactic phenomena, and they work in parallel, unlike the more common, sequential variant of CG.
Functional Dependency Grammar \cite{tapanainen97fdg} is another shallow, reductionistic system,
based on dependency syntax.

Brill tagging \cite{brill1995} is based on transformation rules: the starting point of an analysis is just one, most common tag, and subsequent rule applications transform one tag into another, based on contextual tests.
...
of $\Sigma = {"a", "b"}$ as an input, and returns 
