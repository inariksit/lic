...
Finally, we want to see the model given by the SAT-solver: the disambiguated sentence. We started off with all the variables unassigned, and required at least one variable in each cohort to be true.
In addition, we gave the clause $\laDet \Rightarrow \neg \casaV$. We can see with bare eye that this problem will have a solution; in fact, multiple ones, shown in Figure~\ref{fig:modelsOneRule}.
The verb analysis is removed in the first two columns, as required by the presence of $\laDet$. However, the implication may as well be interpreted ``if $\casaV$ may not follow $\laDet$, better remove $\laDet$ instead''; this has happened in columns 3--4. 
...
But there is power to this property. Now, we add a second rule: \texttt{REMOVE n IF (-1 prn)}, which will form the clause $\laPrn \Rightarrow \neg \casaN \ $. This clause, together with the previous one, $\laDet \Rightarrow \neg \casaV$, prohibits combinations \t{det + v} and \t{prn + n}. The disambiguation is shown in Figure~\ref{fig:modelsTwoRules}.
...
In fact, we have accidentally just implemented FSIG \cite{koskenniemi90}: the rules act in parallel, and if the sentence cannot be fully disambiguated, the remaining uncertainty is modelled as a disjunction of all possible combinations of readings.
In contrast, a sequential CG applies each rule individually, and it cannot handle disjunction; its only operation is to remove\footnote{or add, in CG-3} readings from a cohort. A standard CG engine would have just applied one of the rules---say, the first one, removed the verb and stopped there. If another rule later in the sequence removes the determiner, there is no way to restore the verb. 

To finish our FSIG If we add one more rule, \t{REMOVE v IF (1 adj)}, then we will only get the model in column 1 of Figure~\ref{fig:modelsTwoRules}. Notice that none of the rules has targeted the word \la{}; still it has become unambiguous. 
...
Now, the reader may notice that this is all very nice, but the present thesis is not called ``Analysing FSIG with a SAT-solver''. 
 ...
 \todo{all the magic that happens in the NODALIDA paper \cite{listenmaa_claessen2015}. Also tell that this is exactly what FSIG does, and what \cite{lager98} does. And that we will do more!}
...
The unordered scheme corresponds to Finite State Intersection Grammar (FSIG) \cite{koskenniemi90}, or Parallel Constraint Grammar (PCG), introduced in Section~\ref{sec:ordering}.
