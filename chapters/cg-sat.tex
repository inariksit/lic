...
We can see another benefit in having deeper connections between rules: none of the three rules has targeted the word \la{}, still it has become unambiguous. 
...
Ultimately, solving conflicts means asking the question ``which rule is the best one to apply''
Ordering substitutes the question with ``which rule comes first in the grammar''.
In contrast, the heuristic rule order asks ``out of all the rules that target this cohort, which one has the best matching context?''
...
However, there is a more holistic question: ``out of all the rules that target this cohort, which one is a best fit \emph{with other rules that will apply to this whole sentence}?''
Answering this is beyond the means of previous FSIG implementations \todo{check if so}, but with a SAT-solver, we can answer this question. 
...
Similarly to the order-based heuristic, we create a clause for each 
instance of the rule application. In the case of a conflict, we can 
only discard the clauses targeting the offending cohort, not the whole sentence.
...
Perhaps a more accurate comparison would be a state-of-the-art FSIG parser.
Unfortunately, there have been none since the 1990s \todo{cite!}.
We may still compare the FSIG parsers from 1998 to the CG parsers from 1998:
\cite{voutilainen1998} reports that \todo{the FSIG engine is unable to find a correct parse for some long sentences}; whereas \cite{tapanainen1999phd} reports an average between 2 and 30 seconds for sentences of length 5--50 words; execution time grows in a manner that follows $O(n log\!n)$.
Compared to those results, a modern SAT-based FSIG engine is much closer to a modern CG engine.
