...
\noindent Automatic 0-flagging. 

\noindent Nicer representation of lexical forms.

It could identify pairs for a potential ``feeding order''. Say we have the following rule sequence:
...
Lager \cite{lager2001transformation} observed that the rule sequence learned by the $\mu$TBL system did not remove further ambiguities after its first run, and concluded that the sequence was ``optimal''. 
It would be interesting to recreate the goals in \cite{lager2001transformation} and \cite{bick2013tuning}, to see if this semantic analysis of rule dependencies could lead also to better ordering within a grammar.
