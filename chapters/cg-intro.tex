...
However, even Karlsson's original specifications differ as for the \emph{ordering} of the rules:
...
In the two decades following the initial specification, there have been several independent implementations of CG, with different ordering schemes. In the following section, we will present the different parameters of ordering.
...
A common design pattern is to place more careful rules first; only if they do not 
apply, then try a rule with less conditions.
...
As for existing implementations, \cite{karlsson1990cgp} and \cite{vislcg3} follow the strict scheme, whereas \cite{tapanainen1996} is heuristic.
