...
In the following, we show two variations for emulating order.
In the first one, we show a modification to the parallel scheme, 
where the clauses are added to the formula incrementally, 
making sure that each new clause fits with the previously added.
In the second one, we abandon the parallel scheme, and use \todo{}.



\paragraph{Prefix of rules} We keep the parallel base: the cohort vectors are not manipulated between the rule applications, thus the 100\textsuperscript{th} rule still accesses the same variables as the first rule.
...
For the second option, we abandon the parallel encoding, and return to the semantics of good old sequential CG: rule application takes effect immediately, is irreversible, and only allows changes in target.
To present this scheme, we start from something that is \emph{not} a SAT-problem: namely, the translation into logic by Lager and Nivre \cite{lager_nivre01}.

\todo{present their encoding. The original encoding looks different, but we present their encoding in a notation that is syntactically closer to the one in this chapter.}

Now, it looks pretty much like our previous encoding, so why is this not a SAT-problem? %We sure talk about Boolean values and build formulas.
The answer lies in the first step, where all variables start off as true. All the new variables, created from the rule applications, get their value immediately: $\casa{}'_\v \Leftarrow \grandeAdj \wedge \neg (\casaV \wedge \neg \casaN)$ just translates into $\casa{}'_\v \Leftarrow True \wedge \neg (True \wedge \neg True)$. There is nothing to decide, no search whatsoever; this problem is even simpler than the animal problem, where we at least had some choices to make.
...
Huh. We may see no use for this now. But in Chapter~\ref{chapterCGana}, this question will allow us, ultimately, find conflicts in the grammar; approximate generation, and answer questions about expressivity. 

For the remainder of this chapter, we keep the parallel scheme. In the following sections, we briefly discuss the implementation details of the SAT-CG software, and perform experiments with the two first-mentioned parallel schemes: maximisation-based and order-based.
...
SAT-CG\textsubscript{Max} is the parallel scheme with maximisation and
SAT-CG\textsubscript{Prefix} is the parallel scheme where rules are introduced one by one.
...
As hinted in Section~\ref{sec:orderedScheme}, we will ultimately combine the CG semantics of rule application with the uncertainty ``which analyses did we start with''. 
