...
\item Solve with all clauses: 
  $\{ \ob{\laDet \! \vee \laPrn, \ \casaN \vee \casaV, \  \grandeAdj}^{\text{given by default rule}}, \ 
      \ob{\laDet \Rightarrow \neg \casaV}^{\t{REMOVE v IF (-1 det)}} \}$
\end{enumerate}

\paragraph{Disambiguation}

Finally, we get the model given by the SAT-solver: the disambiguated sentence. 
...
\paragraph{Rule interaction} The reader may notice that this is not a particularly exciting result. A standard CG would just do what it is told and remove the verb, not give 5 different interpretations for a single rule.
But there is power to this property. Now, we add a second rule: \texttt{REMOVE n IF (-1 prn)}, which will form the clause $\laPrn \Rightarrow \neg \casaN \ $. This clause, together with $\laDet \Rightarrow \neg \casaV$, prohibits the combinations $\laDet \ \casaV$  and $\laPrn \ \casaN$; this rules out three models out of five. The disambiguation is shown in Figure~\ref{fig:modelsTwoRules}.
...
In contrast, a sequential CG engine applies each rule individually, and it cannot handle disjunction; its only operation is to remove\footnote{or add, in CG-3} readings from a cohort. The CG engine would have just applied one of the rules---say, the first one, removed the verb and stopped there. If another rule later in the sequence removes the determiner, there is no way to restore the verb. 
...
As an added bonus, none of the three rules has targeted the word \la{}; still it has become unambiguous. 

Now, the reader may notice that this is all very nice, but the present thesis is not called ``Analysing FSIG with a SAT-solver''. 
...
The parallel FSIG has an advantage compared to the sequential CG: it can start disambiguating, even if none of the rules is able to act on its own. 
In addition, the order of the rules does not matter. This prevents too hasty decisions, such as removing $\casaV$ before we know the status of \la{}. 
However, ignoring the order means that we miss significant information in the rule set. 
The truth is that pure FSIG is very brittle: each and every rule in the set must fit together, without the notion of order. The following rule sequence---taken from an actual grammar\footnote{\url{http://link.to/grammar}}---will be well-behaved in a strict and sequential CG. Its behaviour will be less clear in a strict and heuristic CG, but in FSIG, it will definitely cause a conflict, rendering the whole grammar useless.
...
From the success of CG, as opposed to FSIG, one may speculate that the strictly sequential order is easier to understand---undeniably, its behaviour is more transparent. %As opposed to FSIG, the rules are ordered. As opposed to the heuristic order, the rules behave always the same way, regardless of the input.
If two rules target the same cohort, the first mentioned gets to apply, and removes the target. After that, the second one would not even be considered, because of the default rule, which prohibits removing the last reading.
...
Recall the heuristic rule order from Section~\ref{sec:ordering}: a heuristic order asks the question ``out of all the rules that target this cohort, which one is the most exact match?'' If the competitors are \cgrule{REMOVE n IF (-1 prn)} and \cgrule{REMOVE v IF (-1 det) (1 adj)}, then the second one will win. However, if the rules are both as good a match, which happens in Figure~\ref{fig:modelsTwoRules}, we need to resort to mere guessing, or pick the one which is mentioned first in the rule set.
...
With a SAT-solver, we can answer this question. Each rule application produces a formula, which is a conjunction of clauses, and in an ideal case, the whole formula is satisfiable. However, if the whole formula is unsatisfiable, we may still ask for an assignment that satisfies the maximum number of the clauses. 
