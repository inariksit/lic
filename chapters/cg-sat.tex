...
We see a third interpretation in column 5: $\laDet \Rightarrow \neg \casaV$ is, after all, only implication, hence nothing prevents $\casaV$ being removed even without the presence of $\laDet$. 
...
Reader may notice that this is not a particularly exciting result. A standard CG would just do what it is told, not give 5 different interpretations of a single rule.

This is a minimal description of SAT-based implementation. 
Many details are left vague: Do we enforce that all readings that are not targeted by rules will resolve to true? How do we treat multiple rules 





\subsection{Rule application revisited}

All the fancy properties! Flipped rules..... woo-oo, flipp...ed rules. (That doesn't work.)
