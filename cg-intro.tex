...
"<en>"
        "en" det indef utr sg
        "en" noun indef utr sg 
"<bord>"
        "bord" noun indef neutr sg
\end{verbatim}
...
The second word, \emph{bord}, is a neuter noun (`table')---now, the writer has
most likely meant to write ``a table'' instead of ``juniper table'', but has mistaken the gender of `table'.
...
by enumerating only legal combinations (``select \texttt{det neutr} if
+1 is \texttt{n neutr}''), the grammar would just refuse to disambiguate
...
On the other hand, if we redefine \emph{grammatical sequences}
to concern sequences of POS tags, 
the question becomes trivial: this is the whole purpose of CG,
...
a GF grammar which would include the functions \texttt{GenVP} for
...
Both methods have their strengths and weaknesses. A/The strict order is more predictable, but it also means that the grammar writers need to pay more thought to rule interaction. Heuristic order frees the grammar writer from finding an optimal order, but it can give unexpected results, which are harder to debug.
