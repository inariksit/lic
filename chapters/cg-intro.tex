...
The standard measure of formal languages is the Chomsky hierarchy \todo{cite}, with its four classes of grammars and languages, in order from most expressive to least expressive: \emph{recursively enumerable} (Type 0), \emph{context-sensitive} (Type 1), \emph{context-free} (Type 2), and \emph{regular} (Type 3). 
...
\todo{return to this}
Chomsky hierarchy is defined for generative grammar formalisms, hence it makes difficult to place CG directly onto it. 
...
More precisely, a given grammar in CG describes a \emph{relation} between an input language 
and an output language. %: say, $I = \{[\text{"the" }\t{ det def}][\text{"bear" }\t{ noun}, \text{"bear" }\t{ verb}]\}$ and the output is$O =  \{[\text{"the" }\t{ det def}][\text{"bear" }\t{ noun}]\}$.
...
Pasi Tapanainen \cite{tapanainen1999phd} defines the notion \emph{finite-state constraint languages}: the grammar consists of individual rules, which may only contain contextual tests \emph{that are expressed in terms of regular languages}. 
...
Tapanainen gives a hierarchy of four different constraint languages, in order of the expressivity in a single rule: Taggit \cite{taggit} only allows rules to look at 1-2 tokens before or after the target; CG-1 \cite{karlsson1990cgp} allows unlimited context within the window (usually one sentence) and adds operators such as the Kleene star and complement, but it does not allow all combinations. According to Tapanainen \cite{tapanainen1999phd}, a contextual test such as $(\Sigma - N)*A(\Sigma - V)*N$\footnote{IF (*1 A BARRIER N LINK *1 N BARRIER V)} is not supported in CG-1; in turn, CG-2 \cite{tapanainen1996} can express it, but not e.g. repeated patterns, such as $(Det A N)+$. Finally, FSIG \cite{koskenniemi90} allows the full expressivity of regular languages. Since CG-3 \cite{vislcg3,bick2015} did not exist at the time, \cite{tapanainen1999phd} does not include it in the hierarchy of the constraint formalisms.
...
%
%
Closest to answering this question is Anssi Yli-Jyrä
\cite{yli-jyra2003describing,yli-jyra2004fsig_dependency} \todo{cite all relevant Yli-Jyrä},
who investigates the computational complexity of the related formalism FSIG.
%\todo{\cite{yli-jyra2004fsig_dependency} shows that certain FSIGs are closely related to some mildly context-sensitive grammars.} 
